...
class TrainManager:
	...
	def should_train_voidray(self):
		if self.game.can_afford(VOIDRAY):
			if self.game.units(VOIDRAY).amount < self.game.strategy_manager.min_off_vr_rush:
				return True
			if self.game.units(COLOSSUS).amount < self.game.strategy_manager.ground_defenses[COLOSSUS] and self.game.structures(ROBOTICSFACILITY).amount:
				return False
	...
	def should_train_stalker(self):
		if self.game.can_afford(STALKER) and self.game.units(STALKER).amount < self.game.strategy_manager.ground_defenses[STALKER] and not self.game.already_pending(STALKER):
			return True
		return False

	def should_train_sentry(self):
		if self.game.can_afford(SENTRY) and self.game.units(SENTRY).amount < self.game.strategy_manager.ground_defenses[SENTRY] and not self.game.already_pending(SENTRY):
			return True
		return False

	def should_train_colossus(self):
		if self.game.can_afford(COLOSSUS) and self.game.units(COLOSSUS).amount < self.game.strategy_manager.ground_defenses[COLOSSUS] and not self.game.already_pending(COLOSSUS):
			return True
		return False
	...
	async def train_stalker(self):
		for gw in self.game.structures(GATEWAY).ready.idle:
			if self.should_train_stalker():
				self.game.do(gw.train(STALKER))

	async def train_sentry(self):
		for gw in self.game.structures(GATEWAY).ready.idle:
			if self.should_train_sentry():
				self.game.do(gw.train(SENTRY))

	async def train_colossus(self):
		if self.should_train_colossus():
			robotic = self.game.structures(ROBOTICSFACILITY).ready.idle
			if robotic:
				self.game.do(robotic.first.train(COLOSSUS))

	async def train_observer(self):
		if self.should_train_observer():
			robotic = self.game.structures(ROBOTICSFACILITY).ready.idle
			if robotic:
				self.game.do(robotic.first.train(OBSERVER))
