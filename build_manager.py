...
class BuildManager:
	...
	async def build_gateway(self):
		if self.should_build_gateway():
			await self.game.build(GATEWAY, near=self.game.units(PYLON).ready.random)

	async def build_cyberneticscore(self):
		if self.should_build_cyberneticscore():
			await self.game.build(CYBERNETICSCORE, near=self.game.units(PYLON).ready.random)

	async def build_stargate(self):
		if self.should_build_stargate():
			await self.game.build(STARGATE, near=self.game.units(PYLON).ready.random)

	async def build_roboticsfacility(self):
		if self.should_build_roboticsfacility():
			await self.game.build(ROBOTICSFACILITY, near=self.game.units(PYLON).ready.random)
